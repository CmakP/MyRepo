 ...
 * the sequence of nodes. The beginning and ending nodes previous and next 
 ...
 * It can be conceptualized as two singly linked lists formed from the same 
 * data items, but in opposite sequential orders. 
 * 
 * The two node links allow traversal of the list in either direction. While
 * adding or removing a node in a doubly-linked list requires changing more 
 * links than the same operations on a singly linked list, the operations are 
 * simpler and potentially more efficient, because there is no need to keep 
 * track of the previous node during traversal or no need to traverse the list 
